...
| | 100k 基线 | d250 | d500 | d750 | d1000 | d1250（resume250） |
|---|---:|---:|---:|---:|---:|---:|
| F1 | **0.404** | 0.373 | 0.362 | 0.347 | 0.352 | 0.359 |
| AP50 | **0.241** | 0.197 | 0.191 | 0.200 | 0.200 | 0.238 |
| small AP50 | **0.098** | 0.092 | 0.081 | 0.076 | 0.082 | 0.077 |
| SV AP50 | **0.154** | 0.146 | 0.139 | 0.132 | 0.139 | 0.139 |
| AP | 0.101 | 0.094 | 0.098 | 0.093 | 0.103 | **0.107** |

resume 250 步后总指标快速回升（AP50 0.238≈基线、AP 反超），但 **small/SV 仍低于基线**——恢复的是通用检测，不是 RL 目标的密集团小目标。
**终判：现有奖励/协议下 dense GRPO 为净伤害。**（score=1.0 AP 协议下全量绝对值系统性低于 50 张子集，协议效应，只做同协议内比较。）
