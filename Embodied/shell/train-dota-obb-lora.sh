#!/usr/bin/env bash
# DOTA OBB LoRA+LDA training. Recipe must set "geometry": "obb" so the
# trainer adds <obb></obb> tokens and per-sample MTP length 7. CLI
# --block_size stays 6 (HBB datasets in a mixed recipe keep the HBB window).
set -euo pipefail
...
export WANDB_RUN_ID="${WANDB_RUN_ID:-dota-obb-lora-lda}"
...
GPUS=${GPUS:-2}
NNODES=${1:-1}
OUTPUT_DIR=${2:-${OUTPUT_DIR:-"work_dirs/dota_obb_lora_lda_2gpu_4k"}}
NODE_RANK=${NODE_RANK:-0}
PORT=${PORT:-29500}
MASTER_ADDR=${MASTER_ADDR:-"127.0.0.1"}
MODEL_PATH=${MODEL_PATH:-"/root/autodl-tmp/LocateAnything-3B"}
META_PATH=${META_PATH:-"data/dota_v2_obb_448_mix_v1/recipes/dota_v2_obb_448_mix_v1_train_only.json"}
