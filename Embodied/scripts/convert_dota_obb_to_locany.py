#!/usr/bin/env python
"""Convert DOTA OBB annotations to tiled LocateAnything OBB JSONL.

Keeps the HBB converter's CLI, tiling, task-mix, and recipe machinery.
Geometry is a clipped quad → LE90 `(cx,cy,w,h,θ)` → `<obb>` tokens.
"""
...
PROMPT_TEMPLATE = "Locate all oriented instances that match the following description: {classes}."
SINGLE_PROMPT_TEMPLATE = "Locate a single oriented instance that matches the following description: {phrase}."
...
KeptObb = tuple[int, int, int, int, int]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-visibility", type=float, default=0.5, help="Min clipped polygon area/original polygon area")
    parser.add_argument("--min-box-size", type=float, default=2.0, help="Min clipped LE90 width/height in pixels")
    ...
    parser.add_argument("--include-empty", action="store_true", help="Write <obb>none</obb> samples for empty tiles")
...
def parse_dota_label_text(text: str, include_difficult: bool) -> tuple[list[DotaObject], int]:
    ...
    for raw_line in text.splitlines():
        ...
        if max(xs) <= min(xs) or max(ys) <= min(ys):
            continue
        objects.append(DotaObject(label=label, difficult=difficult, quad=tuple(coords)))  # type: ignore[arg-type]
...
def chunked(items: Sequence[tuple[str, KeptObb]], chunk_size: int) -> Iterable[list[tuple[str, KeptObb]]]:
...
def build_answer(boxes: Sequence[tuple[str, KeptObb]]) -> str:
    if not boxes:
        return "<obb>none</obb>"
    parts: list[str] = []
    for label, (cx, cy, w, h, th) in boxes:
        parts.append(f"<ref>{label}</ref><obb><{cx}><{cy}><{w}><{h}><{th}></obb>")
    return "".join(parts)


def make_sample(image_rel: str, boxes: Sequence[tuple[str, KeptObb]]) -> dict:
...
def make_negative_sample(image_rel: str, label: str) -> dict:
    ...
    return {
        "conversations": [
            ...
            {"from": "gpt", "value": "<obb>none</obb>"},
        ],
        "image": image_rel,
    }


def make_referring_sample(image_rel: str, phrase: str, box: KeptObb) -> dict:
    """T5: single-instance referring expression; prompt matches detect_oriented referring style."""
    cx, cy, w, h, th = box
    return {
        "conversations": [
            {"from": "human", "value": SINGLE_PROMPT_TEMPLATE.format(phrase=phrase)},
            {"from": "gpt", "value": f"<ref>{phrase}</ref><obb><{cx}><{cy}><{w}><{h}><{th}></obb>"},
        ],
        "image": image_rel,
    }
...
def intersecting_labels(objects: Sequence[DotaObject], tile_box: tuple[int, int, int, int]) -> set[str]:
    """Labels of every object whose polygon intersects the tile, before visibility filters."""
    labels: set[str] = set()
    for obj in objects:
        pts = np.asarray(obj.quad, dtype=np.float64).reshape(4, 2)
        if clip_polygon_to_rect(pts, tile_box) is not None:
...
def pick_t5(kept: Sequence[tuple[str, KeptObb]]) -> tuple[str, KeptObb] | None:
    ...
    if predicate == "leftmost":
        box = min(boxes, key=lambda b: b[0])
    elif predicate == "rightmost":
        box = max(boxes, key=lambda b: b[0])
    elif predicate == "topmost":
        box = min(boxes, key=lambda b: b[1])
    else:
        box = max(boxes, key=lambda b: b[1])
    ...
    if args.task_mix:
        if split == "train":
            partition_specs: list[tuple[str, str]] = [
                ("train_mix", f"{stem}_train_mix_obb_{args.tile_size}.jsonl"),
                ("internal_val_mix", f"{stem}_internal_val_mix_obb_{args.tile_size}.jsonl"),
            ]
        else:
            partition_specs = [
                ("test_t1", f"{stem}_test_t1_obb_{args.tile_size}.jsonl"),
                ("test_mix", f"{stem}_test_mix_obb_{args.tile_size}.jsonl"),
            ]
    else:
        partition_specs = [(split, f"{stem}_{split}_obb_{args.tile_size}.jsonl")]
        ...
        for record in records:
            ...
            for tile_y in y_starts:
                for tile_x in x_starts:
                    ...
                    for obj in objects:
                        pts = np.asarray(obj.quad, dtype=np.float64).reshape(4, 2)
                        original_area = polygon_area(pts)
                        clipped = clip_polygon_to_rect(pts, tile_box)
                        if clipped is None:
                            continue
                        ...
                        if original_area <= 0 or clipped_area / original_area < args.min_visibility:
                            ...
                            continue
                        cx, cy, w, h, theta = quad_to_le90(clipped)
                        if w < args.min_box_size or h < args.min_box_size:
                            stats.skipped_small_or_invisible_objects += 1
                            continue
                        norm_box = quantize_obb(cx, cy, w, h, theta, tile_x, tile_y, args.tile_size)
                        if norm_box is None:
                            stats.skipped_small_or_invisible_objects += 1
                            continue
    ...
    if not args.dry_run:
        ...
        for partition, _filename in partition_specs:
            ...
            if annotation_path is None:
                continue
            dataset_key = f"{base_key}_{partition}_obb_{args.tile_size}"
...
def write_recipe_and_metadata(args: argparse.Namespace, version: str, recipe_entries: dict, partition_of: dict, stats: list[SplitStats]) -> None:
    ...
    recipe_name = args.recipe_name or f"{args.dota_root.name}_obb_{args.tile_size}"
